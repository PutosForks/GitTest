...
public interface TemplateVerStatusRepository extends JpaRepository <TemplateVerStatus, Long> {
    ...
    @Query("SELECT  new com.example.mfe.pojo.template.templateVerStatusList(tv.idTemplateVer, s1.status, s2.status, s3.status, s1.enviroment, s2.enviroment, s3.enviroment) " +
            "FROM TemplateVer tv " +
